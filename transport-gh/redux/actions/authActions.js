...
export const signupCompany = (newCompanyData, history) => (dispatch) => {
  ...
  axiosNewInstance
    ...
    .then((result) => {
      ...
      dispatch(signupSellerFinal(newCompanyData, history));
    })
    .catch((err) => {
      console.log(err);
    });
...
export const signupCompanyFinal = (newSellerData, history) => (dispatch) => {
  dispatch({ type: LOADING_UI });
  axios
